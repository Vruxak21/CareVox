...
function SuggestedDoctorCard({ doctorAgent, setSelectedDoctor, selectedDoctor }: props) {
  return (
    <div className={`flex flex-col items-center justify-between border rounded-2xl shadow p-5 hover:border-emerald-500 cursor-pointer ${selectedDoctor.id==doctorAgent.id && 'border-emerald-500'}`} onClick={()=>setSelectedDoctor(doctorAgent)}>
      <Image
